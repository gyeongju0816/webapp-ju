...
private List<User> getUsers() {
    ...
    String[] sNames = {"김구", "안창호", "안중근", "유관순", "홍범도", "감좌진", "남자현", "윤봉길", "이봉창", "김원봉"};
    String[] sEmails = {"kim@kku.ac.kr", "an@kku.ac.kr", "an2@kku.ac.kr", "yu@kku.ac.kr", "hong@kku.ac.kr", "kim@kku.ac.kr", "nam@kku.ac.kr", "yun1@kku.ac.kr", "lee@kku.ac.kr", "kim3@kku.ac.kr"};
    ...
    <custom:userCard status="${user.status}" email="${user.email}" name="${user.username}" roles="${user.roles[1]}" id="${user.id}" />
        <%-- <div class="user-card" data-name="${user.username}" data-email="${user.email}" data-role="${user.roles[0]}, ${user.roles[1]}" data-id="${user.id}">
            ...
            <div class="user-info">
                ...
                <button onclick="alert('${user.username}의 상세 정보')">상세 보기</button>
            </div>
        </div> --%>
    </c:forEach>
</div>
